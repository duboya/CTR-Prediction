...
def input_fn(data_file, num_epochs, shuffle, batch_size):
    ...
    def parse_csv(line):
        ...
        columns = tf.decode_csv(line, record_defaults=_CSV_COLUMNS_DEFAULTS)
        # 注意下面组织数据方式，先转换为列表，后转换为dict，方法很巧妙
        features = dict(zip(_CSV_COLUMNS, columns))
        labels = features.pop('income_bracket')
        # tf.equal(x,y) 返回一个bool类型Tensor，表示 x == y，element-wise
        return features, tf.equal(labels, '>50K')

    dataset = tf.data.TextLineDataset(data_file).map(parse_csv, num_parallel_calls=5)

    if shuffle:
        data_file = dataset.shuffle(buffer_size=_NUM_EXAMPLES['train'] + _NUM_EXAMPLES['validation'])
